...
class Application:
    ...
    def tab_help(self):
        ...
        'gage. These values can be used as starting values for HRUs upstream '
        'of the runoff(streamflow gage) site, and the min/max can be used for '
        'calibration bounds in LUCA.\n\n')

        gwflow = (
        'Gwflow Tab\n'
        'Environmental Flow Components tab will create a set of observation '
        'realted files for the purpose of calibration within the LUCA '
        'calibration tool. The file most commonly used from this tab will be '
        'the HIGH6.LOW7 file. LUCA will use this file to examine only '
        'observed and simulated values at either the HIGH or LOW flow regime, '
        'and calibrate related HIGH or LOW flow parameters to those flow '
        'regimes.\n\n')
        ...
        'for fit (4.annually, and 5.all years). This plot is a simple way to '
...
# -----------------------------------------------------------------------------
    ...
    def run_efc(self, tab):
        ...
        # move input file from "efc" folder back to the "input" folder
        iSource = self.appRoot + '\\exe\\efc\\' + tab.filename.get()
        iDestination = self.appRoot + '\\input\\' + tab.filename.get()
        os.rename(iSource, iDestination)

        # navigate python to where the executable outputs the files
        os.chdir('efcs')

        # perminatly move the executable output files
        exeFiles = os.listdir()
    ...
    def run_gwflow(self, tab):
        ...
        # move input file from "gwflow" folder back to the "input" folder
    ...
    def run_makeplot(self, tab):
        ...
        # move input file from "makeplot" folder back to the "input" folder
        iSource = self.appRoot + '\\exe\\makeplot\\' + tab.filename.get()
        iDestination = tab.filepath.get()
        os.rename(iSource, iDestination)

        # move pe file from "makeplot" folder back to the "input" folder
        iSource = self.appRoot + '\\exe\\makeplot\\' + tab.filenamePe.get()
        iDestination = tab.filepathPe.get()
        os.rename(iSource, iDestination)

        # move sr file from "makeplot" folder back to the "input" folder
        iSource = self.appRoot + '\\exe\\makeplot\\' + tab.filenameSr.get()
        iDestination = tab.filepathSr.get()
        os.rename(iSource, iDestination)
